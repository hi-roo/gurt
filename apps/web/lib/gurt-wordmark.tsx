...
// GURT-Wortmarke (FF Unit) als Outline-Pfade — die Render-Quelle der Marke für next/og: Satori
// kann die Typekit-Schrift FF Unit nicht laden, als Pfade erscheint die ECHTE Marke statt des
// Fira-Fallbacks. Pfade identisch zu `app/icon.svg` (Reihenfolge T·R·U·G, Koordinaten im 50×50-
// Raster). Bei Marken-Änderung dort UND hier angleichen.
export const GURT_WORDMARK_PATHS = [
...
// Enge viewBox um die vier Buchstaben (x 6–44,4, y 19–32 im icon.svg-Raster) plus minimaler Rand.
const VIEWBOX = '5 18 40 15';
const RATIO = 40 / 15;

/** GURT-Wortmarke als inline-SVG für OG-Bilder (next/og / Satori). `height` in px, `color` = Textfarbe. */
export function GurtWordmark({ height, color }: { height: number; color: string }): ReactElement {
  return (
    <svg width={Math.round(height * RATIO)} height={height} viewBox={VIEWBOX} xmlns="http://www.w3.org/2000/svg">
      {GURT_WORDMARK_PATHS.map((d) => (
        <path key={d.slice(0, 10)} d={d} fill={color} />
      ))}
    </svg>
  );
}
