...
// Im Konzept liefert in Produktion jeder Beitrag seine eigene Reihe; hier eine echte als Demo.
const FEATURE = {
...
export default async function LabHomePage() {
  ...
  return (
    <>
      {/* Hero — der eine expressive Moment, Text im Bild, ≤ 100vh */}
      <section className="relative isolate flex h-[82vh] max-h-[860px] min-h-[440px] flex-col justify-end overflow-hidden bg-[#0e1326]">
      ...
      <Container width="full">
        {/* „Verstehen"-Suche — prominent, kk.dk-Geste */}
        <Section>
          <p className="font-mono text-xs uppercase tracking-widest text-accent">Was willst du verstehen?</p>
          <Link
            href="/suche"
            className="mt-4 flex items-center gap-4 border border-line px-5 py-5 text-left transition-colors hover:border-ink"
          >
        ...
        {/* „Verstehen"-Index — bedarfsorientiert nach Bereich (kein Feed) */}
        <Section className="border-t border-line">
          <Heading level={2} className="text-2xl">
            Verstehen nach Bereich
          </Heading>
          <div className="mt-6 grid gap-px border border-line bg-line sm:grid-cols-2 lg:grid-cols-4">
            {ressorts.map((r) => (
              <Link key={r.slug} href={`/ressort/${r.slug}`} className="group flex flex-col bg-paper p-5 transition-colors hover:bg-[color-mix(in_srgb,var(--color-accent)_6%,var(--color-paper))]">
                <span className="font-display text-xl font-semibold text-ink group-hover:text-accent">{r.name}</span>
                <span className="mt-1 text-sm text-muted">{RESSORT_DESC[r.slug] ?? ''}</span>
                <span className="mt-6 font-mono text-xs uppercase tracking-widest text-subtle">
                  {r.count} {r.count === 1 ? 'Beitrag' : 'Beiträge'} →
                </span>
              </Link>
            ))}
          </div>
        ...
        {/* Feine Spuren — Themen-Tags als sekundärer, feinerer Einstieg */}
        {themes.length ? (
          <Section className="border-t border-line">
        ...
        {/* Neueste — bewusst sekundär (Index führt, nicht der Feed) */}
        <Section className="border-t border-line">
        ...
        <p className="pb-8 text-sm text-muted">
          Discovery-Notiz: kk.dk-Prinzip — Instrument statt Magazin: „Verstehen"-Index nach Bereich,
          prominente Suche, Themen als feine Spuren, der Feed bewusst sekundär. Accessibility-first
          (Skip-Link, Klartext, Fokus) gehört in die globale Shell. Generatives nur im Hero/Teaser.
        </p>
      </Container>
    </>
  );
