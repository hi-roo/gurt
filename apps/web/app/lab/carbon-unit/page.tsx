 ...
 * — Typografie: FF Unit ("unit", via Typekit). — Farbe: reduziert auf Neutral + EINEN
 *   Kupfer-Ember (Primär). Aqua/Petrol aus dem Interface entfernt. — Hell/Dunkel als
 *   CSS-Variablen-Umschalter. Kontraste beider Themes per Node verifiziert (AA/AAA).
 */
const THEME_CSS = `
...
function UButton({ href, text, kind }: { href: string; text: string; kind: 'onCopperSolid' | 'onCopperGhost' | 'primary' }) {
  ...
  return (
    <a
      href={href}
      style={{ ...style[kind], minWidth: 220, height: 48, paddingLeft: 16, paddingRight: 16, fontWeight: UNIT_W.medium }}
...
export default async function LabCarbonUnitPage() {
  ...
  return (
    <>
      ...
      <CarbonUnitTheme>
        {/* ── UI-Shell-Header ── */}
        <header style={{ height: 48, background: 'var(--cu-surfaceAlt)', borderBottom: '1px solid var(--cu-line)' }} className="sticky top-0 z-40 flex items-stretch">
          <a href="/lab/carbon-unit" className="flex items-center px-4 text-sm transition-colors hover:bg-[var(--cu-hover)]" style={{ borderRight: '1px solid var(--cu-line)' }}>
          ...
          <div className="ml-auto flex items-stretch" style={{ color: 'var(--cu-text2)' }}>
            {['Suche', 'Konto'].map((l) => (
        ...
        <main>
          {/* ── Lead space — asymmetrisch (7/5), Kupfer dominiert, Key-Visual in Kupfer ── */}
          <section className="relative grid lg:grid-cols-[7fr_5fr]" style={{ borderBottom: '1px solid var(--cu-line)' }}>
            <div style={{ background: 'var(--cu-primary)', color: ON_COPPER }} className="flex flex-col justify-between px-6 pt-12 pb-10 sm:px-10 lg:min-h-[34rem] lg:pt-20 lg:pl-16">
              ...
              <h1 style={{ fontWeight: UNIT_W.thin, letterSpacing: '-0.01em', lineHeight: 1.06 }} className="mt-8 max-w-[16ch] text-4xl sm:text-5xl lg:text-6xl">
                Politik verständlich machen, ohne sie einfach zu machen.
              </h1>
              <div className="mt-12 max-w-md lg:ml-16">
            ...
            <div className="relative min-h-[20rem] overflow-hidden lg:min-h-[34rem]" style={{ background: 'var(--cu-bg)' }}>
              ...
              {/* Key-Visual im Primärton (Kupfer) */}
          ...
          {/* ── Tiles — gemischte Größen (breites Feature) ── */}
              ...
              {tiles.map((a, i) => {
                ...
                return (
                  <a
                    key={a.slug}
                    href={`/beitrag/${a.slug}`}
                    className={`group relative flex min-h-[13rem] flex-col justify-between p-5 transition-colors hover:bg-[var(--cu-hover)] ${wide ? 'lg:col-span-2 lg:min-h-[16rem]' : ''}`}
                    style={{ background: copper ? 'var(--cu-primary)' : 'var(--cu-surface)', color: copper ? 'var(--cu-onPrimary)' : 'var(--cu-text)' }}
                  >
          ...
          {/* ── „Thema vorschlagen" — Mitmach-Abschnitt ── */}
          <section id="vorschlag" style={{ background: 'var(--cu-surface)', borderTop: '1px solid var(--cu-line)', borderBottom: '1px solid var(--cu-line)' }}>
            <div className="mx-auto grid max-w-[82rem] gap-10 px-6 py-16 sm:px-10 lg:grid-cols-[5fr_7fr] lg:py-20">
              <div>
                ...
                <p style={{ color: 'var(--cu-text2)' }} className="mt-5 max-w-sm text-base leading-relaxed">
                  GURT lebt von guten Fragen. Welche politische Leitlinie sollen wir uns als Nächstes vornehmen — mit Daten,
                  Quellen und Zielkonflikten?
                </p>
              </div>
              <form action="mailto:hinweise@gurt.info?subject=Themenvorschlag" method="post" encType="text/plain" className="flex flex-col justify-end gap-4">
                <label className="block">
                  ...
                  <input
                    ...
                    style={{ background: 'var(--cu-surfaceAlt)', color: 'var(--cu-text)', borderBottom: '2px solid var(--cu-line)' }}
                    className="mt-2 w-full px-4 py-3.5 text-base outline-none transition-colors placeholder:opacity-60 focus:border-b-[color:var(--cu-primary)]"
                  />
                </label>
